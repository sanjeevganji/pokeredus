// PackIndex — O(1) in-memory lookup layer over a KnowledgePack.
...
export class PackIndex {
  ...
  readonly edges: Map<string, Map<string, Edge>>; // a → b → edge
  ...
  /** All sets grouped by their species id (pokemon_id). */
  readonly setsBySpecies: Map<string, SetEntry[]>;
  /** One representative ("primary") set per species: the Showdown-Usage set when present, else the first. */
  ...
  constructor(pack: KnowledgePack) {
    ...
    // Group sets by species, then pick a deterministic "primary" per species.
  ...
  /** All sets belonging to a species (by pokemon_id). */
  ...
  /** Byte size of the raw pack JSON in MB. */
  ...
  summary(): string {
    return [
      ...
      `#edges=${pack_edgeCount(this)}`,
      `byteSizeMB=${this.byteSizeMB.toFixed(2)}`,
    ].join(', ');
  }
}

function pack_edgeCount(idx: PackIndex): number {
  let n = 0;
  for (const inner of idx.edges.values()) n += inner.size;
  return n;
}
