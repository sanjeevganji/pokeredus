...
import type { Species, SetEntry, Move } from '../src/pack/schema.js';
import { PackIndex } from '../src/pack/index.js';
import { computeDamage, effectiveStat, computeHp } from '../src/engine/damage.js';
import { getEffectiveness } from '../src/engine/type-chart.js';
...
describe('DamageModel', () => {
  describe('effectiveStat / computeHp', () => {
    ...
    it('effectiveStat computes a Special Attack for Venusaur (Timid)', () => {
      ...
      const spa = effectiveStat(venuSet, 'spa', venusaurSp.base_stats, 100);
  ...
  describe('computeDamage — real pack fixtures', () => {
    ...
    it('Flare Blitz (Fire, 2x vs Venusaur Grass) hits with STAB (Arcanine is Fire)', () => {
      ...
      const noCb = (() => {
        // recompute without CB by temporarily zeroing the item
        const arcNoCb: SetEntry = { ...arcSet, item: 'leftovers' };
        return computeDamage(arcNoCb, venuSet, flareBlitz, arcSp, venusaurSp, 100).offensive_stat;
      })();
      expect(r.offensive_stat).toBeGreaterThan(noCb);
      expect(r.final_damage).toBeGreaterThan(0);
    ...
    it('Immune move returns is_immune = true (Psychic vs Dark synthetic)', () => {
      ...
      const darkMon: Species = {
        id: 'synthetic-dark', name: 'Synthetic', types: ['Dark'],
