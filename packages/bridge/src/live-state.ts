...
export class LiveStateWriter {
  ...
  fromDecision(result: DecideResult): void {
    ...
    // #region agent log
    try { fs.appendFileSync(path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../../debug-029c39.log'), JSON.stringify({sessionId:'029c39',runId:'pre',hypothesisId:'E',location:'live-state.ts:fromDecision',message:'eval listings',data:{choiceIds:result.evaluation.choices.map((c)=>c.action.id),replyIds:(result.evaluation.replies??[]).map((r)=>r.action.id),teraChoices:result.evaluation.choices.filter((c)=>c.action.tera||c.action.id.endsWith(':tera')).length,oursHud:this.state.ours.map((s)=>({species:s.speciesId,active:s.active,revealed:s.revealed})),theirsHud:this.state.theirs.map((s)=>({species:s.speciesId,active:s.active,revealed:s.revealed}))},timestamp:Date.now()})+'\n'); } catch { /* ignore */ }
    fetch('http://127.0.0.1:7559/ingest/6200673b-d438-4c7f-9e45-49a0c341555a',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'029c39'},body:JSON.stringify({sessionId:'029c39',runId:'pre',hypothesisId:'E',location:'live-state.ts:fromDecision',message:'eval listings',data:{choiceIds:result.evaluation.choices.map((c)=>c.action.id),replyIds:(result.evaluation.replies??[]).map((r)=>r.action.id),teraChoices:result.evaluation.choices.filter((c)=>c.action.tera||c.action.id.endsWith(':tera')).length},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    this.pushEvent(
